...
function Main() {
  ...
  const changeObject = (event) => {
    ...
    let selectedObjIndex = array.findIndex((item) => item.id === index);
    let newArr = [...array];
    newArr[selectedObjIndex].name = changeInputName;
    newArr[selectedObjIndex].email = changeInputMail;
    setArray(newArr);
    setShowModal(false);
  };
  ...
  return (
    <main>
      <h1 className={flag ? "red" : "blue"}>MAIN</h1>
      <button onClick={addObject}>GET</button>
